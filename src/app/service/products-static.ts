...
export class ProductsStatic {
  ...
  getProductById(prdId:number):Iproducts|undefined{
    return  this.productInService.find((prdInList)=>prdInList.id==prdId)
  }

  doSearch(value: string): Iproducts[] {
    value = value.toLowerCase();
    return this.productInService.filter((prd: Iproducts) =>
...
//1
  // getAllIds():number[]{
  //   return this.productInService.map((_id)=>_id.id)  //[id1.id2,.....]
  // }
}
